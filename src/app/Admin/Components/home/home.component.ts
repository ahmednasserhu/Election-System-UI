...
export class HomeComponentAdmin implements OnInit {
  ...
  fetchTotalCitizens(): void {
    this.citizenService.getCitizenss().subscribe(
      (response) => {
  ...
  fetchRecentActivities(): void {
    this.candidateService.getLastCandidate().subscribe(
      (candidateResponse) => {
    ...
    this.voteService.getLastVote().subscribe(
      (voteResponse) => {
    ...
    this.electionService.getLastElection().subscribe(
      (electionResponse) => {
